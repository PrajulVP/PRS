...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            let exportOptions = {
                columns: [0, 1, 2, 12, 13, 4, 5, 14, 15, 16, 17, 3, 6, 7, 8, 18, 9],
                format: {
                    body: function(data, row, column, node) {
                        ...
                        if (column === 6) { // Total amount column
                            let tableApi = $('#orders-table').DataTable();
                            let rowData = tableApi.row(row).data();
                            return rowData ? parseFloat(rowData.total_amount).toFixed(2) : '0.00';
                        }
                        if (column === 8) { // Placed At
                            let tableApi = $('#orders-table').DataTable();
                            let rowData = tableApi.row(row).data();
                            return rowData ? rowData.placed_at : '';
                        }
                        if (column === 18) { // Delivered At
                            let tableApi = $('#orders-table').DataTable();
                            let rowData = tableApi.row(row).data();
                            return rowData && rowData.delivered_at ? rowData.delivered_at : '';
                        }
