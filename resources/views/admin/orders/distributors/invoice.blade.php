                    ...
                    <td style="padding-left: 15px;">
                        <div class="info-block">
                            ...
                            <div class="info-content">
                                ...
                                {{ $distributorOrder->distributor->address ?? '' }}
                                {{ $distributorOrder->distributor->pincode ?? '' }}<br>
                                Phone:
                                {{ $distributorOrder->distributor->contact_no ?? ($distributorOrder->distributor->phone ?? 'N/A') }}<br>
                                {!! $distributorOrder->distributor->user->email ? 'Email: ' . $distributorOrder->distributor->user->email . '<br>' : '' !!}
                                @if(!empty($distributorOrder->distributor->gst)) GST:
                                {{ $distributorOrder->distributor->gst }}<br> @endif
                                @if(!empty($distributorOrder->distributor->drug_license_no)) DL No:
                                {{ $distributorOrder->distributor->drug_license_no }} @endif
                            </div>
                        </div>
                    </td>
                </tr>
