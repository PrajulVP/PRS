    ...
    <div class="modal fade" id="createFieldStaffModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form id="createFieldStaffForm" action="{{ route('admin.field-staffs.store') }}" method="POST" novalidate autocomplete="off">
                    @csrf
    ...
    <div class="modal fade" id="editFieldStaffModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form id="editFieldStaffForm" method="POST" novalidate>
                    @csrf
                    @method('PUT')
                    <div class="modal-body">
                        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="status" id="edit_status" class="form-select" {{ Auth::user()->hasAnyRole(['admin', 'superadmin']) ? '' : 'disabled' }}>
    ...
    <script>
        ...
        $(document).ready(function () {
                    ...
                    if (response.success) {
                            ...
                            brandTargetsHtml += `
                                <div class="col-sm-4">
                                    <div class="p-2 border rounded border-primary h-100" style="background: var(--med-bg-body);">
                                        ...
                                        <div class="d-flex justify-content-between small">
                                            <span class="text-muted">Target:</span>
                                            <span class="fw-semibold" style="color: var(--med-text-main);">₹${parseFloat(fs.current_month_target_amount || 0).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                                        </div>
                                        <div class="d-flex justify-content-between small">
                                            <span class="text-muted">Achieved:</span>
                                            <span class="fw-semibold text-success">₹${parseFloat(fs.achieved_target || 0).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                                        </div>
                                    </div>
                                </div>
                            `;
                        ...
                        let targetsHtml = fs.sales_targets?.map(target => {
                            ...
                            return `<tr>
                                ...
                                <td>₹${parseFloat(target.amount).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
                                <td>₹${parseFloat(target.achieved_amount).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-6">
                            <div class="quick-card">
                                ...
                                <div class="value text-warning">${parseFloat(data.loyalty_points || 0).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="quick-card">
            ...
            $('input[name="password"]').on('input blur', function(e) {
                ...
                let errorDiv = $(this).parent().hasClass('password-field-container') ? $(this).parent().next('.invalid-feedback') : $(this).next('.invalid-feedback');
                if (errorDiv.length === 0) {
                    let target = $(this).parent().hasClass('password-field-container') ? $(this).parent() : $(this);
                    target.after('<div class="invalid-feedback d-block pass-error"></div>');
                    errorDiv = target.next('.pass-error');
                }
            ...
            $('input[name="password_confirmation"]').on('input blur', function() {
                ...
                if (errorDiv.length === 0) {
                    let target = $(this).parent().hasClass('password-field-container') ? $(this).parent() : $(this);
                    target.after('<div class="invalid-feedback d-block confirm-error"></div>');
                    errorDiv = target.next('.confirm-error');
                }
